...
func NewRedisCache(rdb *redis.Client) (Cache, error) {
...
func (c *redisCache) GetStringSortedWithScores(key, min, max string) []ZSetWithScoreData {
	results := c.rdb.ZRangeByScoreWithScores(context.Background(), key, &redis.ZRangeBy{
		Min: min,
		Max: max,
	}).Val()
	parsedResults := make([]ZSetWithScoreData, 0, len(results))
	for _, v := range results {
		value := ZSetWithScoreData{}
